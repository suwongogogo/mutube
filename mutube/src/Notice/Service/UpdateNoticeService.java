	...
	public void update(NoticeData noticeData) throws UpdatePostFailExcpetion, SQLException{
		try(Connection conn = ConnectionProvider.getConnection()){
			...
			if( noticeData.getNoticeContent().getImageNames()!= null) {
				...
				noticeContent = new NoticeContent(noticeData.getNotice().getNoticeId(), noticeData.getNoticeContent().getContent(), noticeData.getNoticeContent().getVideo_link(), imageName );
				updateCnt = noticeContentDAO.updateWithImage(conn, noticeData.getNoticeContent(), noticeData.getNotice().getNoticeId());
			}else {
				noticeContent = new NoticeContent(noticeData.getNotice().getNoticeId(), noticeData.getNoticeContent().getContent(), noticeData.getNoticeContent().getVideo_link());
				updateCnt = noticeContentDAO.updateNoticeContent(conn, noticeData.getNoticeContent(), noticeData.getNotice().getNoticeId());
			}
